...
import { History, Trash2, X, Clock, TableProperties } from "lucide-react";
import { FieldRequestEntry, formatHistoryDate, clearFieldHistory } from "@/lib/history";
...
interface Props {
  ...
  onSelect: (entry: FieldRequestEntry) => void;
...
export function FieldHistoryPanel({ entries, onSelect, onDelete, onClearAll, open, onClose }: Props) {
  if (!open) return null;
      ...
      <div className="fixed top-0 right-0 z-40 h-full w-80 flex flex-col bg-primary border-l border-theme shadow-2xl">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-theme bg-panel flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-primary">Field Request History</span>
          </div>
          ...
          {entries.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-center">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-600">
                Requests are saved automatically when you download
              </p>
            </div>
          ) : (
                ...
                <li key={entry.id} className="group relative">
                  ...
                  >
                    ...
                    <p className="text-xs text-primary font-medium truncate">
                      {entry.tableName || "(no table)"}
                    </p>
                    <p className="text-[10px] text-secondary mt-0.5">
                      {entry.rows.length} field{entry.rows.length !== 1 ? "s" : ""} · {entry.date}
                    </p>
                  </button>
