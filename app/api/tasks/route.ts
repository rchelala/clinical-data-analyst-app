...
export async function GET(req: NextRequest) {
  try {
    ...
    if (hasAssignedTo && hasExcludeWorklistOf) {
      const rows = await sql`
        SELECT
          t.id, t.dashboard_id, t.subscription_id, t.division_id, t.owner_analyst_id, t.created_by_id, t.title, t.description,
    ...
    return NextResponse.json(
      {
        error:
          'Provide dashboardId, subscriptionId, or divisionId (optionally with ownerAnalystId), or both assignedTo and excludeWorklistOf.',
      },
      { status: 400 }
    );
  } catch (err: unknown) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { dashboardId, subscriptionId, divisionId, title, description, status, priority, ownerAnalystId } = body;
    ...
    if (Number(hasDashboardId) + Number(hasSubscriptionId) + Number(hasDivisionId) !== 1) {
      return NextResponse.json(
        { error: 'Provide exactly one of dashboardId, subscriptionId, or divisionId.' },
        { status: 400 }
      );
    }
    ...
    const rows = await sql`
      INSERT INTO tasks (dashboard_id, subscription_id, division_id, owner_analyst_id, created_by_id, title, description, status, priority)
      VALUES (${dashboardId ?? null}, ${subscriptionId ?? null}, ${divisionId ?? null}, ${ownerAnalystId ?? createdById}, ${createdById}, ${title}, ${description ?? null}, ${status ?? 'open'}, ${priority ?? null})
    ...
    if (err && typeof err === 'object' && 'code' in err) {
      ...
      if (code === '23503') {
        return NextResponse.json(
          { error: 'Referenced dashboardId/subscriptionId/divisionId/ownerAnalystId does not exist' },
          { status: 400 }
        );
      }
      if (code === '23514') {
