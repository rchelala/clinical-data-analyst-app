...
export async function GET(req: NextRequest) {
  try {
    ...
    return await getMonthlySummary(monthParam);
  } catch (err: unknown) {
...
async function getMonthlySummary(month: string): Promise<NextResponse> {
  ...
  const response: MonthlySummaryResponse = { month, totals, divisions };
