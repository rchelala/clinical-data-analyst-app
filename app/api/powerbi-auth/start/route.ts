...
export async function POST() {
  try {
    ...
    if (!res.ok) {
      ...
      console.error("Power BI device code request failed:", res.status, text);
      return NextResponse.json(
        { error: "Power BI sign-in failed. Please try again." },
        { status: 502 }
      );
    }
    ...
    console.error("Power BI device code request error:", err);
    return NextResponse.json(
      { error: "Power BI sign-in failed. Please try again." },
      { status: 500 }
    );
