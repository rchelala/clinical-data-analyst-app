...
export async function GET(req: NextRequest) {
  try {
    ...
    // Batch mode: all requests for every dashboard/subscription in a
    // division, in one query, instead of the caller fetching per-entity.
    // Same row shape (and mapper) as the dashboardId/subscriptionId paths
    // below, so callers can group client-side by dashboardId/subscriptionId.
    if (divisionIdParam !== null) {
      ...
      const rows = await sql`
        ...
        WHERE r.dashboard_id IN (SELECT id FROM dashboards WHERE division_id = ${divisionId})
           OR r.subscription_id IN (SELECT id FROM report_subscriptions WHERE division_id = ${divisionId})
