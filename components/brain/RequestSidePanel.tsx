...
// (which is wired for an editable dropdown) — this is a simpler, display-only
// subset covering the common task statuses, with a neutral fallback.
...
export function RequestSidePanel({
  ...
  // Read-only "Tasks" section state — only fetched for dashboards (kind ===
  // "dashboard"), since subscriptions don't have tasks. Kept separate from
  // the requests state above since it's a distinct, non-interactive list.
  ...
  // (like the tags fetch below) — names just won't resolve.
